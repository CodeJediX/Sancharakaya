(() => {
  ...
  function renderChatSessions(){const select=$("#chatSessionSelect");if(!select)return;const sessions=chatSessions();select.innerHTML=sessions.map(s=>`<option value="${esc(s.id)}" ${s.id===currentChatId?"selected":""}>${esc(s.title||"Untitled chat")}</option>`).join("")||"<option>No previous chats</option>";select.disabled=!sessions.length;$("#deleteChatBtn").disabled=!sessions.length;$("#newChatBtn").disabled=false}
  function persistActiveChat(){if(!currentChatId)return;const sessions=chatSessions();const i=sessions.findIndex(s=>s.id===currentChatId);if(i<0)return;const firstUser=chatHistory.find(m=>m.role==="user")?.content;const autoTitle=["New Sri Lanka trip","First Sri Lanka trip","New chat"].includes(sessions[i].title);sessions[i]={...sessions[i],title:autoTitle?makeChatTitle(firstUser||sessions[i].title):sessions[i].title,messages:chatHistory,updatedAt:Date.now(),memory:{...memory}};saveChatSessions(sessions);renderChatSessions();renderProfile()}
  function createChat(seed="New Sri Lanka trip"){const sessions=chatSessions();const session={id:String(Date.now()),title:makeChatTitle(seed),createdAt:Date.now(),updatedAt:Date.now(),messages:[],memory:{...memory}};sessions.unshift(session);saveChatSessions(sessions);currentChatId=session.id;saveJSON(userKey("currentChatId"),currentChatId);chatHistory=[];$("#chatLog").innerHTML="";addMessage("bot","New chat started. Tell me what kind of Sri Lanka trip you want.");renderChatSessions();renderProfile()}
  ...
  function deleteCurrentChat(){if(!currentChatId)return;const sessions=chatSessions().filter(s=>s.id!==currentChatId);saveChatSessions(sessions);currentChatId=sessions[0]?.id||null;if(currentChatId)loadChat(currentChatId);else createChat("New Sri Lanka trip")}
  ...
  function initAuth(){currentUser=loadJSON("sancharakayaCurrentUser",null);afterSignIn()}
  ...
  function signOut(){currentUser=null;currentChatId=null;chatHistory=[];localStorage.removeItem("sancharakayaCurrentUser");$("#chatLog").innerHTML="";afterSignIn()}
  function renderAuthState(){const signed=!!currentUser;document.querySelector(".chat-panel")?.classList.remove("chat-locked");$("#chatGate")?.classList.add("unlocked");const mini=$("#authMiniCard");if(mini){mini.classList.toggle("signed-in",signed);mini.innerHTML=signed?`<strong>${esc(currentUser.name)}</strong><span>Assistant chats save to your traveler profile.</span>`:`<strong>Guest assistant</strong><span>Login on the main website to attach chats to your traveler profile.</span>`}$("#sendBtn").disabled=false;renderChatSessions()}
